...
export const useSession = () => {
  ...
  if (!context) {
    throw new Error(
      "The useAuth() hook must be wrapped within a <SessionProvider />"
    );
  }
  return context;
};
...
export const SessionProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [sendResetPasswordLoading, setSendResetPasswordLoading] =
    useState(false);
  ...
  const signUp = useSignUpHelper({
    onError: (message, errors) => {
      Alert.alert("Can't Register", message, [{ text: "OK" }]);
    },
    onFinish: () => {
  ...
  const signIn = useSignInHelper({
    onError: (message, errors) => {
      Alert.alert("Can't Log In", message, [{ text: "OK" }]);
    },
    onFinish: () => {
  ...
  const signOut = useSignOutHelper({
    onError: (message, errors) => {
      Alert.alert("Can't Log Out", message, [{ text: "OK" }]);
    },
    onFinish: () => {
  ...
  const { verifyEmail, sendVerificationEmail } = useEmailVerificationHelper({
    verifyEmailOption: {
      onError: (message, errors) => {
        Alert.alert("Can't Verify Your Email", message, [{ text: "OK" }]);
      },
      onFinish: () => {
    ...
    sendVerificationEmailOption: {
      onError: (message, errors) => {
        Alert.alert("Email Not Sent", message, [{ text: "OK" }]);
      },
      onFinish: () => {
  ...
  const { resetPassword, sendResetPassword } = usePasswordResetHelper({
    resetPasswordOption: {
      onError: (message, errors) => {
        Alert.alert("Can't Reset Password", message, [{ text: "OK" }]);
      },
      onFinish: () => {
    ...
    sendResetPasswordOption: {
      ...
      onFinish: () => {
        setSendResetPasswordLoading(false);
      },
      onStart: () => {
