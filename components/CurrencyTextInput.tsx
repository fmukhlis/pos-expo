import { View, Text, NativeSyntheticEvent, TextInputKeyPressEventData, TextInputFocusEventData } from 'react-native'
...
const CurrencyTextInput = ({
    ...
    const currentValue = React.useMemo(() => {
        ...
        const formatted = numberFormat.format(Math.min(Number(raw), 999999999))
        return formatted
    }, [value])
...
interface CurrencyTextInput extends Omit<React.ComponentPropsWithoutRef<typeof PrimaryInput>, 'value'> {
...
const generateFormattedValue = (prevValue: string, newValue: string, type: 'add' | 'remove' | 'set') => {
    switch (type) {
        case 'add': {
            let raw = `${prevValue}${newValue}`
            const formatted = numberFormat.format(Number(raw))
