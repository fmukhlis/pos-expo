...
const PaymentMethodModal = ({
  ...
  const paymentMethodId = useAppSelector(
    ({ paymentMethod }) => paymentMethod.selectedPaymentMethodId
  );

  const [getPaymentMethod, getPaymentMethodResult] =
    useLazyGetPaymentMethodQuery();
  const [storePaymentMethod, storePaymentMethodResult] =
    useStorePaymentMethodMutation();
  const [updatePaymentMethod, updatePaymentMethodResult] =
    useUpdatePaymentMethodMutation();
  ...
  React.useEffect(() => {
    if (paymentMethodId) {
      getPaymentMethod({ paymentMethodId, storeId })
        .unwrap()
        .then(({ id, ...rest }) => {
          setData(rest);
        })
      ...
      <View className="flex-1 items-center justify-center">
        ...
        >
          {getPaymentMethodResult.isFetching ? (
            <View>
              <CustomActivityIndicator size={"large"} className="m-auto" />
            </View>
          ) : (
            <>
              ...
              />
              <View className="flex-row items-center justify-end space-x-3 mt-2">
                <SecondaryButtonSM
                  onPress={() => {
                    onClose();
                  }}
                ...
                <PrimaryButtonSM
                  isProcessing={
                    storePaymentMethodResult.isLoading ||
