...
import { useDestroyPaymentMethodMutation } from "../services/paymentMethod";
...
const DeletePaymentMethodButton = ({
  ...
  const [destroyPaymentMethod, destroyPaymentMethodResult] =
    useDestroyPaymentMethodMutation();

  const handleDelete = () => {
    destroyPaymentMethod({ paymentMethodId, storeId });
  };
  ...
  return (
    ...
    >
      {({ pressed }) => {
        return destroyPaymentMethodResult.isLoading ? (
