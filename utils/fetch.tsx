...
export const useCustomFetch = () => {
  ...
  const request = async (relativeURL: string, init: RequestInit = {}) => {
    const dynamicUrl =
      (await SecureStore.getItemAsync("apiBaseUrl")) ?? baseURL;
    const { method = "GET", headers, ...rest } = init;
    const mergedUrl = dynamicUrl + relativeURL;
    const mergedHeaders = {
      ...baseHeaders,
      ...headers,
    };
  ...
  return {
     ...
     */
    get: (relativeURL: string, init: Omit<RequestInit, "method"> = {}) =>
      request(relativeURL, { method: "GET", ...init }),
    /**
     ...
     * @returns A promise that resolves to HTTP response
     */
    post: (relativeURL: string, init: Omit<RequestInit, "method"> = {}) =>
      request(relativeURL, { method: "POST", ...init }),
    /**
     ...
     * @returns A promise that resolves to HTTP response
     */
    patch: (relativeURL: string, init: Omit<RequestInit, "method"> = {}) =>
      request(relativeURL, { method: "PATCH", ...init }),
    /**
     ...
     * @returns A promise that resolves to HTTP response
     */
    put: (relativeURL: string, init: Omit<RequestInit, "method"> = {}) =>
      request(relativeURL, { method: "PUT", ...init }),
    /**
     ...
     * @returns A promise that resolves to HTTP response
     */
    delete: (relativeURL: string, init: Omit<RequestInit, "method"> = {}) =>
      request(relativeURL, { method: "DELETE", ...init }),
  };
};
