    ...
    <ScrollView className="flex-1 bg-white pt-[70]">
      <View className="flex-row flex-wrap gap-2 p-2">
        {items.map((item, index) => (
          <View
            key={item.id}
            className={`basis-[40%] flex-grow h-24 bg-gray-100 border border-gray-300 rounded justify-center items-center`}
          >
            <Text className="text-lg font-bold text-gray-500/80">
              {item.name}
            </Text>
          </View>
        ))}
