...
import { useDestroyProductMutation, useLazyGetProductQuery, useStoreProductMutation, useUpdateProductMutation } from '@/components/services/product'
...
const ManageProduct = () => {
    ...
    const storeId = selectedStore ? selectedStore.id : 0
    ...
    const changeSku = (newSku: string) => {
        dispatchData({ type: 'UPDATE_VARIANT', index: 0, value: { sku: newSku.toUpperCase() } })
    }
    const changePrice = (value: { raw: string; formatted: string }) => {
        dispatchData({ type: 'UPDATE_VARIANT', index: 0, value: { price: value.raw } })
    }
    const changeStock = (newStock: string) => {
        dispatchData({ type: 'UPDATE_VARIANT', index: 0, value: { stock: `${isNaN(parseInt(newStock, 10)) ? '' : parseInt(newStock, 10)}` } })
    }

    const submit = () => {
        if (validateData(data)) {
            if (productId) {
                updateProduct({ storeId, productId, ...data }).unwrap()
                    .then(() => {
                        if (!updateResult.isError) { shouldGoBackRef.current = true }
                    })
                    .catch((error) => {
                        Alert.alert(
                            'Error ' + error.status,
                ...
                createProduct({ storeId, ...data }).unwrap()
                    .then(() => {
                        if (!createResult.isError) { shouldGoBackRef.current = true }
                    })
                    .catch((error) => {
                        Alert.alert(
                            'Error ' + error.status,
            ...
            [
                { text: 'Cancel' },
                {
                    text: 'Confirm', onPress: () => {
                        if (productId) {
                            deleteProduct({ productId, storeId }).unwrap().then(() => { if (!deleteResult.isError) { shouldGoBackRef.current = true } })
                        }
                    }
                }
            ]
    ...
    const [selectedModifierIndex, setSelectedModifierIndex] = React.useState<null | number>(null)
    const hideManageModifierModal = () => { setModalVisibility((prev) => ({ ...prev, manageModifier: false })) }
    const showManageModifierModal = () => { setModalVisibility((prev) => ({ ...prev, manageModifier: true })) }
    const handleManageModifierModalSave = (value: Exclude<ProductPayload["availableModifiers"], null>[number]) => {
        if (typeof selectedModifierIndex === 'number') {
            dispatchData({ type: 'UPDATE_MODIFIER', index: selectedModifierIndex, value })
        } else {
    ...
    const [selectedOptionIndex, setSelectedOptionIndex] = React.useState<null | number>(null)
    const hideManageOptionModal = () => { setModalVisibility((prev) => ({ ...prev, manageOption: false })) }
    const showManageOptionModal = () => { setModalVisibility((prev) => ({ ...prev, manageOption: true })) }
    const handleManageOptionModalSave = (value: Exclude<ProductPayload["availableOptions"], null>[number]) => {
        if (typeof selectedOptionIndex === 'number') {
            dispatchData({ type: 'UPDATE_OPTION', index: selectedOptionIndex, value })
        } else {
    ...
    const hideManageVariantModal = () => { setModalVisibility((prev) => ({ ...prev, manageVariant: false })) }
    const showManageVariantModal = () => { setModalVisibility((prev) => ({ ...prev, manageVariant: true })) }
    const handleManageVariantModalSave = (value: ProductPayload["availableVariants"][number]) => {
        dispatchData({ type: 'UPDATE_VARIANT', index: selectedVariantIndex, value })
    ...
    React.useEffect(() => {
        if (storeId && productId) {
            triggerGetProduct({ storeId, productId }).unwrap()
                .then((product) => {
                    if (product) {
                        const { name, availableModifiers, availableOptions, availableVariants } = product
                        dispatchData({
                            type: 'SET',
                            value: {
                                name,
                                availableModifiers: availableModifiers.length
                                    ? availableModifiers
                                        ...
                                        .map(({ name, values }) => ({
                                            categoryName: name,
                                            values: values.filter(({ status }) => (status === 'Active')).map(({ name }) => ({ name }))
                                        }))
                                    : null,
                                ...
                                availableVariants: availableVariants
                                    ...
                                    .map(({ price, productOptions, sku, stock }) => ({
                                        options: productOptions.length ? productOptions.map(({ name }) => (name)) : null,
                                        price: `${price}`,
                                        sku,
                                        stock: `${stock}`
                                    }))
    ...
    return (
        <ScrollView className='flex-1 bg-white px-4'>
            <ManageModifierModal
                ...
                initialData={typeof selectedModifierIndex === 'number' ? data.availableModifiers?.[selectedModifierIndex] : null}
                onSave={handleManageModifierModalSave}
                onClose={hideManageModifierModal}
            />
            <ManageOptionModal
                visible={modalVisibility.manageOption}
                initialData={typeof selectedOptionIndex === 'number' ? data.availableOptions?.[selectedOptionIndex] : null}
                        ...
                        <Modifier
                            ...
                            onPressRemove={() => {
                                Alert.alert(
                                    ...
                                    [
                                        { text: 'CANCEL' },
                                        { text: 'REMOVE', onPress: () => { dispatchData({ type: 'REMOVE_MODIFIER', index }) } }
                                    ]
                                )
                            }}
                        />
                    ))
                    : <Text className='mt-1 text-gray-400'>You don't set the modifier sets yet.</Text>
                }
            </View>
            <View className='mt-4'>
                <SecondaryButton
            ...
            <View className='mt-5'>
                ...
                <Text className='mt-1 mb-2'>
                    Add a custom set of options to create variations for an item.
                    For example, add a size option to create variations for small, medium and large.
                </Text>
                        ...
                        <Option
                            ...
                            onPressRemove={() => {
                                Alert.alert(
                                    ...
                                    [
                                        { text: 'CANCEL' },
                                        { text: 'REMOVE', onPress: () => { dispatchData({ type: 'REMOVE_OPTION', index }) } }
                                    ]
                                )
                            }}
                        />
                    ))
                    : <Text className='mt-1 text-gray-400'>You don't set the options yet.</Text>
                }
            </View>
            <View className='mt-4'>
                <SecondaryButton
            ...
            {data.availableOptions?.length
                ?
                <View className='mt-5'>
                    <Text className='text-base font-medium mb-1.5'>
                        Variants
                    </Text>
...
type Action =
    ...
    | { type: 'ADD_MODIFIER'; value: Exclude<ProductPayload['availableModifiers'], null>[number] }
    | { type: 'UPDATE_MODIFIER'; index: number; value: Partial<Exclude<ProductPayload['availableModifiers'], null>[number]> }
    | { type: 'REMOVE_MODIFIER'; index: number }
    | { type: 'ADD_OPTION'; value: Exclude<ProductPayload['availableOptions'], null>[number] }
    | { type: 'UPDATE_OPTION'; index: number; value: Partial<Exclude<ProductPayload['availableOptions'], null>[number]> }
    | { type: 'REMOVE_OPTION'; index: number }
    | { type: 'UPDATE_VARIANT'; index: number; value: Partial<ProductPayload['availableVariants'][number]> }
    | { type: 'RESET_VARIANT' }
...
const dataReducer = (
    ...
    switch (action.type) {
        ...
        case 'ADD_MODIFIER': {
            const dirtyModifiers = [...(state.availableModifiers ?? []), action.value]
            // This will ensures that the modifier category is unique. 
            // The duplicated one will be overwritten.
            const cleanedModifiers = Array.from(new Map(dirtyModifiers.map(
                (modifier) => ([modifier.categoryName, modifier])
            )).values())
        ...
        case 'UPDATE_MODIFIER': {
            if (state.availableModifiers?.length) {
                const dirtyModifiers = state.availableModifiers.map((
                    ...
                    index === action.index
                        ? {
                            categoryName: action.value.categoryName ?? availableModifier.categoryName,
        ...
        case 'ADD_OPTION': {
            ...
            // The duplicated one will be overwritten.
            const cleanedOptions = Array.from(new Map(dirtyOptions.map(
                (option) => ([option.categoryName, option])
            )).values())
            return { ...state, availableOptions: cleanedOptions, availableVariants: generateVariants(cleanedOptions) }
        }
        case 'UPDATE_OPTION': {
            if (state.availableOptions?.length) {
                const dirtyOptions = state.availableOptions.map(
                    (availableOption, index) => (
                        index === action.index
                            ? {
                                categoryName: action.value.categoryName ?? availableOption.categoryName,
...
const generateVariants = (options: Exclude<ProductPayload['availableOptions'], null>) => {
    if (options.length) {
