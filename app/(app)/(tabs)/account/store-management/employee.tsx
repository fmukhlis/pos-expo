...
const Employee = () => {
    ...
    const [filteredEmployees, setFilteredEmployees] = React.useState<EmployeeProps[]>([])

    const [
        getEmployeesLoading,
        setGetEmployeesLoading
    ...
    const [inviteEmployeeModalVisible, setInviteEmployeeModalVisible] = React.useState(false)
    ...
    const reloadScreensData = () => {
        ...
        getOutgoingEmployeeInvitations({
            ...
            onSuccess: (invitations) => {
                setEmployeeInvitations(invitations.filter((invitation) => (invitation.status != 'Accepted')))
            },
            onFinish: () => {
    ...
    return (
        <View className='flex-1 bg-white p-4 relative'>
                ...
                <FlatList
                                    ...
                                    onPress={() => {
                                        Alert.alert(
                                            ...
                                            [
                                                ...
                                                { text: 'OK', onPress: () => { terminate(item.id) } }
                                            ]
                                        )
                                    }}
                                >
                                    ...
                                    >
                                        ...
                                        <Icon name='radio-button-on' className={`${item.status === 'Active' ? 'text-green-500' : 'text-gray-400'}`} size={20} />
                                    </View>
                                </TouchableOpacity>
                            </View>
                        )
                        ...
                        <View className=''>
                            <View className='flex-row justify-between items-center mb-3'>
                                ...
                                <View className='w-[25] h-[25] rounded-full bg-rose-600'>
                                    <Text className='m-auto text-[11px] font-bold text-white'>{employees.length < 100 ? employees.length : '99+'}</Text>
                                </View>
                            </View>
                            <PrimaryInput
                    ...
                    ListEmptyComponent={
                        <View className='border-t py-3 border-gray-300'>
                            <Text className='text-gray-500 text-center'>You don't have any employee.</Text>
                        </View>
                    }
                    refreshControl={
                        <RefreshControl
            ...
            <View
                className='absolute h-[200] bottom-0 left-0 right-0 m-4'
            >
                <FlatList
                    renderItem={({ item }) => {
                        return (
                                ...
                                <TouchableOpacity
                                    onPress={() => {
                                        Alert.alert(
                                            ...
                                            [
                                                { text: 'Cancel', style: 'cancel' },
                                                { text: 'OK', onPress: () => { disinvite(item.id) } }
                                            ]
                                        )
                                    }}
                                >
                    ...
                    ListEmptyComponent={
                        <View className='border-t py-3 border-gray-300'>
                            <Text className='text-gray-500 text-center'>You haven't invited anyone yet.</Text>
                        </View>
                    }
                />
            </View>
